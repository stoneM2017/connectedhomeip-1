...
void MemMonitoring::HeapMonitoring(void * pvParameter)
{
    ...
    while (1)
    {
        ...
        ChipLogDetail(NotSpecified, "\r\n");
        ChipLogDetail(NotSpecified, "SRAM Heap, min left: %d, current left %d", 
            xPortGetMinimumEverFreeHeapSize(), xPortGetFreeHeapSize());
        ChipLogDetail(NotSpecified, "PSRAM Heap, min left: %d, current left %d", 
            xPortGetMinimumEverFreeHeapSizePsram(), xPortGetFreeHeapSizePsram());
        ChipLogDetail(NotSpecified, "============================= / total run time %ld", pulTotalRunTime);

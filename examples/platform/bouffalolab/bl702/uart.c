...
static int uartRecvIntr(void *p_arg);

void uartInit(void)
{
    hosal_uart_callback_set(&uart_stdio, HOSAL_UART_RX_CALLBACK, uartRecvIntr, NULL);
...
static void uartRecvTimeout(void)
{
...
static int uartRecvIntr(void *p_arg)
{
    uint32_t len = 0, expect = 0;

    bl_timer_stop(CHIP_UART_RX_TIMER_ID);

    if (chipUart_var.head >= chipUart_var.tail) {
        expect = MAX_BUFFER_SIZE - chipUart_var.head;
    ...
    len = hosal_uart_receive(&uart_stdio, chipUart_var.rxbuf + chipUart_var.head, chipUart_var.tail);
    chipUart_var.head += len;

exit:

    if ((chipUart_var.head + 1) % MAX_BUFFER_SIZE == chipUart_var.tail) {
        uartRecvTimeout();
    }
    else {
        bl_timer_start(CHIP_UART_RX_TIMER_ID, bl_timer_get_current_time() + 10 * 1000, uartRecvTimeout);
    }
    return 0;
}
...
{
    return hosal_uart_send(&uart_stdio, Buf, BufLength) == 0? BufLength: 0;
}
